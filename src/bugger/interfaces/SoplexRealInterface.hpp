...
namespace bugger
{
   ...
   public:
         ...
         if( retcode > SPxSolver::NOT_INIT )
         {
            ...
            if( this->parameters.mode == -1 )
            {
               ...
               if( retcode == SolverRetcode::OKAY && dual && this->soplex->isDualFeasible() )
                  retcode = this->check_dual_bound( this->soplex->objValueReal(), std::max(this->soplex->realParam(SoPlex::FEASTOL), this->soplex->realParam(SoPlex::OPTTOL)), this->soplex->realParam(SoPlex::INFTY) );
               ...
               if( retcode == SolverRetcode::OKAY && ( primal && this->soplex->isPrimalFeasible() || objective ) )
               {
                  ...
                  this->soplex->getPrimalReal(solution[0].primal.data(), solution[0].primal.size());

                  for( int col = solution[0].primal.size() - 1, var = this->soplex->numCols(); col >= var; --col )
                  ...
                  if( this->soplex->hasPrimalRay() )
                  {
                     ...
                     this->soplex->getPrimalRayReal(solution[0].ray.data(), solution[0].ray.size());

                     for( int col = solution[0].ray.size() - 1, var = this->soplex->numCols(); col >= var; --col )
                  ...
                  if( primal )
                     retcode = this->check_primal_solution( solution, std::max(this->soplex->realParam(SoPlex::FEASTOL), this->soplex->realParam(SoPlex::OPTTOL)), this->soplex->realParam(SoPlex::INFTY) );
               }
               ...
               if( retcode == SolverRetcode::OKAY && objective )
               {
                  ...
                  retcode = this->check_objective_value( this->soplex->objValueReal(), solution[0], std::max(this->soplex->realParam(SoPlex::FEASTOL), this->soplex->realParam(SoPlex::OPTTOL)), this->soplex->realParam(SoPlex::INFTY) );
               }
            }
         }
         else
      ...
      readInstance(const String& settings_filename, const String& problem_filename) override
      {
         ...
         for( int col = 0; col < ncols; ++col )
         {
            ...
            builder.setObj(col, this->soplex->objReal(col));
   ...
   private:
      ...
      setup(SolverSettings& settings, const Problem<REAL>& problem, const Solution<REAL>& solution)
      {
         ...
         this->set_parameters( );
         this->soplex->setRealParam(SoPlex::OBJ_OFFSET, obj.offset);
         ...
         for( int row = 0; row < nrows; ++row )
         {
            ...
            this->soplex->addRowReal(LPRow(lhs, cons, rhs));
         ...
         if( this->initial )
         {
            if( solution_exists )
            {
               ...
               for( int index = 0; index < doublesettings.size( ); ++index )
               {
                  ...
                  {
                     this->soplex->setRealParam(param, std::max(std::min(this->soplex->realParam(param) - this->value, this->soplex->realParam(SoPlex::INFTY)), -this->soplex->realParam(SoPlex::INFTY)));
         ...
         if( solution_exists )
         {
            if( abs(this->soplex->realParam(SoPlex::OBJLIMIT_LOWER)) < this->soplex->realParam(SoPlex::INFTY) )
               this->soplex->setRealParam(SoPlex::OBJLIMIT_LOWER, std::max(std::min(this->soplex->realParam(SoPlex::OBJLIMIT_LOWER) + this->value, this->soplex->realParam(SoPlex::INFTY)), -this->soplex->realParam(SoPlex::INFTY)));
            if( abs(this->soplex->realParam(SoPlex::OBJLIMIT_UPPER)) < this->soplex->realParam(SoPlex::INFTY) )
               this->soplex->setRealParam(SoPlex::OBJLIMIT_UPPER, std::max(std::min(this->soplex->realParam(SoPlex::OBJLIMIT_UPPER) + this->value, this->soplex->realParam(SoPlex::INFTY)), -this->soplex->realParam(SoPlex::INFTY)));
            if( this->parameters.set_dual_limit || this->parameters.set_prim_limit )
            {
               for( const auto& pair : this->limits )
               {
                  ...
                  {
                  case DUAL:
                     this->soplex->setRealParam(SoPlex::RealParam(pair.first.back()), this->relax( this->value, obj.sense, 2 * std::max(this->soplex->realParam(SoPlex::FEASTOL), this->soplex->realParam(SoPlex::OPTTOL)), this->soplex->realParam(SoPlex::INFTY) ));
                     break;
                  case PRIM:
                     this->soplex->setRealParam(SoPlex::RealParam(pair.first.back()), this->value);
                     break;
                  }
               }
            }
